...
def agregar_persona():
    ...
    db.session.commit()
    return jsonify({'message': 'Se agrego una persona'}), 201
...
def obtener_persona():
    ...
    persona_json = [{'nombre': persona.nombre, 'apellido': persona.apellido,
                     'email': persona.email, 'dni': persona.dni}for persona in persona]
...
def modificar_persona():
    ...
    if dni:
        ...
        if persona:
            ...
            db.session.commit()
            return jsonify({'message': 'Se modifico persona'}), 200
        return jsonify({'message': 'Persona no encontrada'}), 404
...
def eliminar_persona(dni):
    ...
    if persona:
        ...
        db.session.commit()
        return jsonify({'message': 'Se elimino una persona correctamente'}),200
    return jsonify({'message': 'Persona no encontrada'}), 404
